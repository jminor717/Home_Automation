...
#define UPPER_TENS 1
#define UPPER_ONES 3
#define UPPER_TENTHS 11

#define LOWER_TENS 5
#define LOWER_ONES 7
#define LOWER_TENTHS 9
...
void EPD_Temperature1(I2CBus* bussin) { bussin->write(adds_com, new uint8_t[3] { 0x7E, 0x81, 0xB4 }, 3); }
...
public:
    ...
    void IncrementTest(void)
    {
        ...
        float temp = 0;
        if (displayedUpper < 20 || displayedUpper > 90) {
            temp = 20.7;
        } else {
            temp = displayedUpper + 1;
        }
        SetUpperDisplayFloat(temp);
    }
    ...
    void DecrementTest(void)
    {
        // WriteScreen(DSPNUM_1in9_off);
        uint8_t setpoint = 0;
        if (displayedLower < 20 || displayedLower > 90) {
            setpoint = 20;
        } else {
            setpoint = displayedLower + 1;
        }
        float spSim = ((float)setpoint) + 0.3;
        SetLowerDisplayInt(spSim);
    }
