...
CONF_POWER_DISPLAY = "bat_power"
CONF_I_CHARGE = "charge_current"
    ...
    cv.Schema(
        {
            ...
            cv.Required(CONF_V_BAT_SENSOR): cv.use_id(sensor.Sensor),
            cv.Required(CONF_I_BAT_SENSOR): cv.use_id(voltage_sampler.VoltageSampler),
            cv.Required(CONF_I_CHARGE): cv.use_id(sensor.Sensor),
            ...
            cv.Optional(CONF_POWER_DISPLAY): sensor.sensor_schema(
...
async def to_code(config):
    ...
    cg.add(var.set_Battery_Current_Sensor(i_bat_sens))

    i_charge_sens = await cg.get_variable(config[CONF_I_CHARGE])
    cg.add(var.set_Charge_Current_Sensor(i_charge_sens))
    ...
    if CONF_POWER_DISPLAY in config:
        power_sensor = await sensor.new_sensor(config[CONF_POWER_DISPLAY])
        cg.add(var.set_power_display(power_sensor))