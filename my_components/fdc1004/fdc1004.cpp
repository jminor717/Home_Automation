...
static const uint8_t CHANEL_CONFIG_ADR[] = { 0x08, 0x09, 0x0A, 0x0B };
static const uint8_t MEASUREMENT_MSB_ADR[] = { 0x00, 0x02, 0x04, 0x06 };
static const uint8_t MEASUREMENT_LSB_ADR[] = { 0x01, 0x03, 0x05, 0x07 };
...
void FDC1004::write16(uint8_t reg, uint16_t data)
{
    ...
    auto err = this->write_register(reg, array, 2);
    if (err != i2c::ERROR_OK) {
...
uint16_t FDC1004::read16(uint8_t reg)
{
    ...
    uint8_t array[1] = { 0 };
    auto err = this->write_register(reg, array, 0);
    ...
    uint8_t bytes[2] = { 0, 0 };
    bool err = this->read_bytes_raw(bytes, 2);
